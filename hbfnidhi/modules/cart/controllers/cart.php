...
class Cart extends Public_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array('products/product_model','members/members_model','cart_model','adminzone/news_model'));		 
		...
		//	{
				...
				if( $this->auth->is_user_logged_in() )
				{
					  ...
					  {	
					   ...
					   echo  redirect($this->session->userdata('ref'),'');	
	...
	private function add_customer_order($costumer_data = array(),$is_same_bill_ship)
	{
		if( $this->cart->total_items() > 0 )
		{
			...
			$invoice_number    = "E-coachingIndia_".get_auto_increment('tbl_order');	
			...
			$tax_res    =  get_db_single_row($fld='tbl_tax','tax_rate,tax_type',$Condwherw="WHERE tax_id='1'");
			...
			{//echo "<pre>";	print_r($data_order);die;
				$orderId = $this->cart_model->safe_insert('tbl_order',$data_order,FALSE);		
...
$data = array(
				 ...
				 $this->cart_model->safe_insert('tbl_orders_products',$data,FALSE);
				...
				{				
					...
					$data = array('status'=>'1');
					$this->cart_model->safe_update('tbl_coupon_customers',$data,$where,FALSE);
   ...
   {	
	...
	private function add_cart()
	{
		...
		if( is_array($pres) && !empty($pres))
		{
			...
			$qty         = ($qty > 0) ? $qty: 1;		
			$cart_price  = ( $pres['lecture_discounted_price']!= '0.0000') ? $pres['lecture_discounted_price'] : $pres['lecture_price'];
			...
			//trace($pres);die;
				...
				'price'          => $cart_price,
				'product_price'  => $pres['lecture_price'],
				'discount_price' => $pres['lecture_discounted_price'],
				'name'           => url_title($pres['lecture_name']),
				'origname'       => $pres['lecture_name'],												
				'pid'            => $pres['products_id'],												
				'img'            => $pres['lecture_image'],												
				'code'           =>$pres['lecture_code']
				);
									
				$ins = $this->cart->insert($cart_data);
	...
	{ 	
		 $binary_data =  get_db_field_value('tbl_orders_products','product_image',array('orders_products_id'=>$orders_products_id));
	...
	public function course_checkout()
	{
			$productId  = (int) $this->uri->segment(3);
		    $is_same_bill_ship =   $this->input->post('is_same',TRUE);	   
		    $mres = $this->members_model->get_member_row( $this->session->userdata('user_id') );			
		...
		{			
			...
			{				
				...
				$data['mres'] = $mres;				
				$this->load->view('view_course_checkout',$data);	
				
			}else
			{				
				$this->add_customer_order1($posted_data,$productId);
			} 
		...
		private function add_customer_order1($costumer_data = array(),$pid)
	{
			...
			$currency_value    = $this->session->userdata('currency_value');
			$customers_id   =  ( $userId!='') ? $userId : 0;
		    $res = $this->cart_model->getcouse_detail($id);
				
	if($res['discounted_price'] > 0 && $res['discounted_price']  < $res['price']  )
	{
		$amt= $res['discounted_price'] ;
	}
		else{
			$amt = $res['price'] ;
		}
...
$data = array(
							'orders_id'      => $orderId,							
							'products_id'    => $res['combo_Id'],
							'product_name'   => $res['Combo_title'],
							'product_code'   => 'N/A',
							'product_image'  => $res['combo_img'],							
							'product_price'  => $amt,						
							'quantity'       => '0',
	                        'type'      => 'full'
			  );
	...
	public function invoice1()
	{
		if( $this->session->userdata('working_order_id') > 0 )
		{
			...
			$data['ordId']      = $order_res['order_id'];			
			$data['unq_section'] = "Checkout";	
			$this->load->view('view_cart_invoice_course',$data);
	...
	public function print_invoice_detail()
	{
		$this->load->model(array('order/order_model'));
		$ordId  = $this->uri->segment(3,$this->session->userdata('working_order_id'));
		...
		$data['ordId'] = $ordId;		 
		$this->load->view('view_invoice_print1',$data);
	}
	public function display_cart_image1($orders_products_id)
	{ 	
		 $binary_data =  get_db_field_value('tbl_categories','category_image',array('category_id'=>$orders_products_id));
		//echo $binary_data;
		//die('sunil');
		/// header("Content-Type: image/jpeg");			 
		// echo $binary_data; 
