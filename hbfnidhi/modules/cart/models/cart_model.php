...
class cart_model extends MY_Model {
	...
	public function getcouse_detail($id)
	{
		$data = array('combo_Id'=>$id);
		 
		 $que = $this->db->get_where('tbl_combo_pack',$data);
	...
	{
		$num =  $this->findCount('tbl_order',"invoice_number = '$ord' ");     
	...
	public function get_discount($code)
	{
		...
		{		
		  ...
		  $res = $this->find('tbl_coupons',$fetch_config);		
	      return $res;
		}
		...
		public function get_shipping_rate($shipId)
		{
			...
			if($id!='' && is_numeric($id))
			{
				...
				$result = $this->find('tbl_shipping',$fetch_config);
				return $result;		
			}
		}
	...
	public function add_wislists($prodId,$memId)
	{
		if($prodId > 0 && $memId > 0 )
		{
				$record =  $this->is_record_exits('tbl_wishlists',array('condition'=>"customer_id =$memId AND product_id =$prodId") );
			...
			if(!$record )
			{
				...
				$this->safe_insert('tbl_wishlists',$data,FALSE);
