...
class Gallery extends Admin_Controller
{
	...
	{		
		...
		{			
			$this->update_status('tbl_gallery','gallery_id');			
		}
	...
	{		  
		$data['heading_title'] = 'Add Gallery';	
		 ...
		 $this->form_validation->set_rules('gallery_title','Gallery Title',"required|max_length[99]|unique[tbl_gallery.gallery_title='".$this->input->post('gallery_title')."' AND tbl_gallery.status!='2']");
		...
		if($this->form_validation->run()==TRUE)
		{
		    ...
		    $this->gallery_model->safe_insert('tbl_gallery',$posted_data,FALSE);									
	...
	public function edit()
	{
		...
		{
				$this->form_validation->set_rules('gallery_title','Gallery Title',"required|max_length[99]|unique[tbl_gallery.gallery_title='".$this->input->post('gallery_title')."' AND tbl_gallery.gallery_id!='".$rowdata->gallery_id."' AND tbl_gallery.status!='2']");
				...
				if($this->form_validation->run()==TRUE)
				{
					...
					$this->gallery_model->safe_update('tbl_gallery',$posted_data,$where,FALSE);						
					$this->session->set_userdata(array('msg_type'=>'success'));				
