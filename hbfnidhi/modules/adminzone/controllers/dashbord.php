...
class Dashbord extends Admin_Controller {
  ...
  public function __construct() {
    ...
    public  function index()
	{
		...
		$data['total_order']  = $this->count_record(' tbl_order',"order_status !='deleted' ");
		$data['total_member'] = $this->count_record('tbl_customers',"status !='2' ");
		$data['total_testimonial'] = $this->count_record('tbl_testimonial',"status !='2' ");			
		$this->load->view('dashboard/dashbord_index_view',$data);	
        
	  // $table_fields = $this->table_info('tbl_testimonial');
	  // trace($table_fields);
