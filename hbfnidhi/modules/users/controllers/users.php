...
class Users extends Public_Controller
{
	...
	public function __construct()
	{
		...
		$this->load->model(array('users/users_model','pages/pages_model','adminzone/news_model'));	
	...
	{		    
		...
		{			
		   $is_same_bill_ship =   $this->input->post('is_same',TRUE);
			...
			$this->form_validation->set_rules('phone_number', 'Phone','trim|required|max_length[25]|xss_clean');
			//$this->form_validation->set_rules('last_name', 'Last Name', 'trim|required|alpha|max_length[32]|xss_clean');	
					

			$this->form_validation->set_rules('billing_name', 'Billing Name','trim|required|max_length[32]|xss_clean|alpha');
			$this->form_validation->set_rules('billing_phone', 'Billing Phone','trim|required|max_length[25]|xss_clean');
			...
			$this->form_validation->set_rules('billing_zipcode', 'Billing Post Code','trim|max_length[25]|required|xss_clean');
			
			
			
			if( $is_same_bill_ship!='Y')
			{
				$this->form_validation->set_rules('shipping_name', 'Shipping Name','trim|required|max_length[32]|xss_clean|alpha');
				$this->form_validation->set_rules('shipping_phone', 'Shipping Phone','trim|required|max_length[25]|xss_clean');
				...
				$this->form_validation->set_rules('shipping_city', 'Shipping City','trim|max_length[50]|required|xss_clean');
				$this->form_validation->set_rules('shipping_zipcode', 'Shipping Post Code','trim|max_length[25]|required|xss_clean');
			}
			
			$this->form_validation->set_rules('terms_conditions', 'Terms and Condition','trim|required|xss_clean');
